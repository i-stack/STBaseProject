...
4. 测量工具
   - `STMarkdownTextViewMeasure.measure` 可用 TextKit 1 的 `layoutManager.usedRect(for:)` 得到内容高度。
   - 高度按 `gridSize` 向上取整，默认 2pt，用于减轻亚像素抖动。
5. 高度通知
...
| 高度粒度 | `ceil(height)` | `ceil` 或 `gridSize` 向上取整 |
