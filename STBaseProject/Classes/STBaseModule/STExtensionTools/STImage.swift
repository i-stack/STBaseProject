...
// MARK: - 兼容性扩展（保持向后兼容）
public extension UIImage {
    @available(*, deprecated, message: "使用 isEmpty(_:) 替代")
    static func st_imageIsEmpty(image: UIImage) -> Bool {
        return isEmpty(image)
    }
    
    @available(*, deprecated, message: "使用 toBase64() 替代")
    func st_imageToBase64() -> String {
        return toBase64()
    }
    
    @available(*, deprecated, message: "使用 toData() 替代")
    func st_imageToData() -> Data {
        return toData()
    }
    
    @available(*, deprecated, message: "使用 getFormat() 替代")
    func st_getImageFormat() -> STImageFormat {
        return getFormat()
    }
    
    @available(*, deprecated, message: "使用 getTypeString() 替代")
    func st_getImageType() -> String? {
        return getTypeString()
    }
    
    @available(*, deprecated, message: "使用 getColor(at:) 替代")
    func st_getPointColor(point: CGPoint) -> UIColor {
        return getColor(at: point)
    }
    
    @available(*, deprecated, message: "使用 smartCompress(_:maxFileSize:quality:format:) 替代")
    static func st_compressImageToSize(_ image: UIImage, maxFileSize: Int) -> Data? {
        return smartCompress(image, maxFileSize: maxFileSize)
    }
    
    @available(*, deprecated, message: "使用 resizeImage(_:to:) 替代")
    static func st_resizeImage(_ image: UIImage, targetSize: CGSize) -> UIImage? {
        return resizeImage(image, to: targetSize)
    }
    
    @available(*, deprecated, message: "使用 getLaunchImage() 替代")
    static func st_getLaunchImage() -> UIImage {
        return getLaunchImage()
    }
    
    @available(*, deprecated, message: "使用 calculateTargetSize(for:maxFileSize:) 替代")
    static func st_calculateTargetSize(for image: UIImage, maxFileSize: Int) -> CGSize {
        return calculateTargetSize(for: image, maxFileSize: maxFileSize)
    }
}
