...
public enum STTimeProfiler {
    ...
    public static func st_start(tag: String = "default") {
        self.lock.lock()
    ...
    public static func st_end(tag: String = "default", message: String? = nil) {
        self.lock.lock()
        defer { self.lock.unlock() }
    ...
    public static func st_elapsedTime(tag: String = "default") -> Double? {
        self.lock.lock()
        defer { self.lock.unlock() }
        guard let startTime = self.startTimes[tag] else {
            return nil
    ...
    ///   - tag: 计时任务标识，默认为 "default"
    ///   - message: 自定义消息
    public static func st_logElapsed(tag: String = "default", message: String? = nil) {
    ...
    /// - Returns: 代码块的返回值
    @discardableResult
    public static func st_measure<T>(tag: String = "default", message: String? = nil, block: () throws -> T) rethrows -> T {
    ...
    ///   - message: 自定义消息
    ///   - block: 要执行的异步代码块
    public static func st_measureAsync(tag: String = "default", message: String? = nil, block: @escaping () async throws -> Void) {
        self.st_start(tag: tag)
        Task {
            do {
                try await block()
    ...
    /// 清除所有计时任务
    public static func st_clearAll() {
        self.lock.lock()
        defer { self.lock.unlock() }
    ...
    public static func st_clear(tag: String) {
        self.lock.lock()
        defer { self.lock.unlock() }
        self.startTimes.removeValue(forKey: tag)
        STLog("🧹 已清除计时任务: \(tag)")
    ...
    /// - Returns: 格式化后的字符串
    private static func st_formatDuration(_ duration: Double) -> String {
        if duration < 0.001 {
            // 小于1毫秒，显示微秒
