...
public extension STHTTPSession {
    ...
    func st_request<T: Codable>(
        ...
        self.request(
            ...
            completion(STHTTPResponseWithModel<T>(data: response.data, response: response.response, error: response.error, model: model))
        }
    }
    ...
    func st_upload(
        ...
        if let progress = progress {
            request.uploadProgress(handler: progress)
        }
    ...
    func st_download(
        ...
        let request = self.download(url, to: destinationURL)
        if let progress = progress {
